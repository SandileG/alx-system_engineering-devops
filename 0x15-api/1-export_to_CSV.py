...
def fetch_todo_list(user_id):
    """Fetches the to-do list for the given user ID from the JSONPlaceholder API."""
...
if __name__ == "__main__":
    ...
    with open(f"{user_id}.csv", "r") as csv_file:
        ...
        for index, row in enumerate(reader, start=1):
            expected_row = [user_data["id"], user_data["username"], str(todo_list[index - 1]["completed"]), todo_list[index - 1]["title"]]
            if row != expected_row:
